...
// Returns a *NotSingularError when exactly one Participant entity is not found.
...
// Returns a *NotSingularError when exactly one Participant ID is not found.
...
//		GroupBy(participant.FieldCreated).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Participant.Query().
//		Select(participant.FieldCreated).
//		Scan(ctx, &v)
//
func (pq *ParticipantQuery) Select(fields ...string) *ParticipantSelect {
	pq.fields = append(pq.fields, fields...)
	return &ParticipantSelect{ParticipantQuery: pq}
}
...
func (pq *ParticipantQuery) sqlAll(ctx context.Context) ([]*Participant, error) {
	...
	if query := pq.withEvent; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_participants" returned %v`, n.ID)
			}
			for i := range nodes {
