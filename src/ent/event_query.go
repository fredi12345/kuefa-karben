...
func (eq *EventQuery) QueryParticipants() *ParticipantQuery {
	query := &ParticipantQuery{config: eq.config}
...
func (eq *EventQuery) QueryImages() *ImageQuery {
	query := &ImageQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) WithImages(opts ...func(*ImageQuery)) *EventQuery {
	query := &ImageQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withParticipants; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Participants = []*Participant{}
		}
		query.withFKs = true
		query.Where(predicate.Participant(func(s *sql.Selector) {
			s.Where(sql.InValues(event.ParticipantsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_participants" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_participants" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Participants = append(node.Edges.Participants, n)
		}
	}
	...
	if query := eq.withComments; query != nil {
		...
		for _, n := range neighbors {
			fk := n.event_comments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_comments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_comments" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Comments = append(node.Edges.Comments, n)
		}
	}

	if query := eq.withImages; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Event)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Image(func(s *sql.Selector) {
			s.Where(sql.InValues(event.ImagesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.event_images
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_images" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
