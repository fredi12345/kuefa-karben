...
func (iq *ImageQuery) QueryEvent() *EventQuery {
	query := &EventQuery{config: iq.config}
...
func (iq *ImageQuery) WithEvent(opts ...func(*EventQuery)) *ImageQuery {
	query := &EventQuery{config: iq.config}
...
func (iq *ImageQuery) sqlAll(ctx context.Context) ([]*Image, error) {
	...
	if query := iq.withEvent; query != nil {
		...
		for i := range nodes {
			if nodes[i].event_images == nil {
				continue
			}
			fk := *nodes[i].event_images
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_images" returned %v`, n.ID)
			}
			for i := range nodes {
